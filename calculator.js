...
container.setAttribute(
  "class",
  "container d-flex flex-column flex-content-center"
);

container.innerHTML = `
    <h1 id="title" class="text-center">Calculator</h1>
 <p id="description" class="text-center">Perform All you calculation at one place</p>
...
function isNumberkey(event) {
  let restrictElement = document.getElementById("restrictToast");

  let restrictToast = new bootstrap.Toast(restrictElement, {
    delay: 3000,
  });
...
//function to diaply the value in input box
...
function calculate() {
  ...
  try {
    outputDisplay.value = eval(outputDisplay.value).toFixed(3);
  } catch (error) {
    invalidToast.show();
  }
}
